...
import { updateVerificationDocument, nextStep, previousStep } from "@/store/selfSlice";
...
const VerificationDocumentPage: React.FC = () => {
  ...
  const { verificationDocument } = useSelector((state: RootState) => state.selfOnboarding);
  ...
  const handleNext = () => {
    if ( !verificationDocument.medicalDegreeCertificate) {
      alert("Please upload at least Government ID and Medical Degree Certificate");
      return;
    }
    console.log(verificationDocument)
    dispatch(nextStep());
    router.push("/clinic-onboarding/self-onboarding/work-preferences");
  };
