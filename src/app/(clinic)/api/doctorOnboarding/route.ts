...
export async function POST(request: NextRequest) {
  try {
    ...
    const doctorData = {
      ...
      associatedClinicHospitalName: formData.get('associatedClinicHospitalName') as string || '',
      consultationType: formData.get('consultationType') as string || '',
      availableConsultationHours: formData.get('availableConsultationHours') as string || '',
      preferredModeOfConsultation: formData.get('preferredModeOfConsultation') as string || '',
    ...
    const medicalDegree = formData.get('medicalDegreeCertificate') as File | null;
    if (medicalDegree && medicalDegree instanceof File && medicalDegree.size > 0) {
    ...
    const registrationCert = formData.get('medicalCouncilRegistrationCertificate') as File | null;
    if (registrationCert && registrationCert instanceof File && registrationCert.size > 0) {
    ...
    if (experienceCert && experienceCert instanceof File && experienceCert.size > 0) {
    ...
    const doctorOnboardingData = {
      ...
      uploadedFiles: {
        profilePhoto: documents.profilePhotoUrl ? 'Uploaded successfully' : 'Not provided',
        governmentId: documents.governmentIdUrl ? 'Uploaded successfully' : 'Not provided',
        medicalDegree: documents.medicalDegreeUrl ? 'Uploaded successfully' : 'Not provided',
        registrationCert: documents.registrationCertUrl ? 'Uploaded successfully' : 'Not provided',
        experienceCert: documents.experienceCertUrl ? 'Uploaded successfully' : 'Not provided',
      }
    };
    ...
    return NextResponse.json(
      {
        success: true,
        message: 'Doctor onboarding data received, files uploaded to Cloudinary, and saved successfully',
